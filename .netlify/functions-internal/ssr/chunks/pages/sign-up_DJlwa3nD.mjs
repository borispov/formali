...
const $$SignUp = createComponent(async ($$result, $$props, $$slots) => {
  ...
  console.log(answer, isInterested);
  ...
  return renderTemplate`${maybeRenderHead()}<div class="content opacity-100 relative w-full max-w-md px-4 h-full md:h-auto mx-auto" dir="rtl"> <!-- Modal content --> <div class="flex items-center justify-center flex-col bg-white rounded-lg relative dark:bg-gray-700"> <div class="flex justify-end p-2"> <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white" data-modal-toggle="authentication-modal"></button> </div> <form name="sign-up" method="POST" action="/thanks" data-netlify="true" class="space-y-6 px-6 lg:px-8 pb-4 sm:pb-6 xl:pb-8"> ${void 0} <h3 class="text-xl font-medium text-gray-900 dark:text-white"> ${welcomeTitle} </h3> <p class="text-gray-600 text-sm"> ${paraText} </p> ${isInterested && renderTemplate`<div> <label for="email" class="text-sm font-medium text-gray-900 block mb-2 dark:text-gray-300">כתובת דוא״ל</label> <input type="email" name="email" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="name@company.com" required=""> </div>
...
export { $$SignUp as default, $$file as file, partial, prerender, $$url as url };
