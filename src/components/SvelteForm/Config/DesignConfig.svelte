<script lang="ts">
    ...
    let {
        ...
        onDesignEdit,
        formDesign = $bindable(),
    }: { formDesign: FormDesign } = $props();

    let d = $derived(formDesign);

    $effect(() => {
        onDesignEdit("question", d.question);
    });
    $effect(() => {
        onDesignEdit("answer", d.answer);
    });
    $effect(() => {
        onDesignEdit("button", d.button);
    });
    $effect(() => {
        onDesignEdit("buttonText", d.buttonText);
    });
        ...
        {#each themes as theme, index}
            ...
            >
                <h3
                    ...
                    class={`text-sm text-[${theme.question}]`}
                >
                    {theme.name}
                </h3>
                <button
                    style="background-color: {theme.button}; color: {theme.buttonText};"
                    class={`w-12 rounded-md btn text-[10px] btn-xs`}
                    >המשך</button
                >
            </div>
        {/each}
        ...
        <ConfigSetting label={LABELS.FONTSIZE}>
            <div class="flex gap-2 items-center px-2 join">
                ...
                <div class="join-item has-[:checked]:bg-gray-100 p-2">
                    ...
                    <label
                        for="descFontMd"
                        class="peer-checked:bg-orange-600 block i-mdi:size-m h-2rem w-2rem cursor-pointer"
                    ></label>
                </div>
                ...
                <div class="join-item has-[:checked]:bg-gray-100 p-2">
                    ...
                    <label
                        for="descFontLg"
                        class="peer-checked:bg-orange-600 block i-mdi:size-l h-2rem w-2rem cursor-pointer"
                    ></label>
                </div>
            </div>
        </ConfigSetting>
        ...
        <ConfigSetting label={LABELS.ALIGN}>
            <div class="flex gap-2 items-center px-2 join">
                ...
                <div class="join-item has-[:checked]:bg-gray-100 p-2">
                    ...
                    <label
                        for="descAlignCenter"
                        class="peer-checked:bg-blue-600 block i-mdi:format-align-center h-1.5rem w-1.5rem cursor-pointer"
                    ></label>
                </div>
                ...
                <div class="join-item has-[:checked]:bg-gray-100 p-2">
                    ...
                    <label
                        for="descAlignEnd"
                        class="peer-checked:bg-blue-600 block i-mdi:format-align-left h-1.5rem w-1.5rem cursor-pointer"
                    ></label>
                </div>
            </div>
        </ConfigSetting>
