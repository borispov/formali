<script>
    ...
    async function onSubmitHandler(e) {
        ...
        try {
            ...
            if (err.data.message.toLowerCase().includes("auth")) {
                error = "כתובת מייל או סיסמא שגויים!";
            } else {
                error = "חלה שגיאה בניסיון התחברות, אנא נסה שוב מאוחר יותר.";
            }
        }
    }
</script>
    ...
    <div class="flex flex-row flex-wrap justify-center items-center gap-20">
        ...
        >
            <div class="flex flex-col space-y-2 text-center">
                ...
                >
                <h1 class="text-2xl font-semibold tracking-tight">התחברות</h1>
                <p class="text-sm text-muted-foreground">
                    התחבר באמצעות אימייל וסיסמא
                </p>
            </div>
            <div class="grid gap-6">
                ...
                <div class="relative">
                    <div class="absolute inset-0 flex items-center">
                        <span class="w-full border-t bg-yellow-500 text-yellow"
                        ></span>
                    </div>
